...
public class MinigamePickLock : MonoBehaviour
{
    [System.Serializable]
    public class Bar
    {
    ...
    void GeneratePin()
    {
        ...
        for (int i = 0; i < bars.Count; i++)
        {
            ...
            bar.greyPin.sizeDelta = new Vector2(bar.greyPin.sizeDelta.x, bar.greyLength * stepHeight);
            if (bar.greyPin != null)
            {
    ...
    void UpdateVisual()
    {
        for (int i = 0; i < bars.Count; i++)
        {
            ...
            if (bar.greyPin != null)
            {
                ...
                s.y = height;
                bar.greyPin.sizeDelta = s;
                bar.greyPin.anchoredPosition = new Vector2(bar.greyPin.anchoredPosition.x, bottomY);
            }
            if (bar.redPin != null)
            {
                bar.redPin.anchoredPosition = new Vector2(bar.redPin.anchoredPosition.x, (bar.redPosition - 0.47f) * stepHeight);
            }
            if (bar.completed)
            {
